...
class ResUsersRole(models.Model):
    ...
    group_id = fields.Many2one(
        ...
        readonly=True, string=u"Associated group", hide=True)
    ...
    group_category_id = fields.Many2one(
        related='group_id.category_id', readonly=True,
        default=lambda cls: cls.env.ref(
            'base_user_role.ir_module_category_role').id)
    group_category_ids = fields.Many2many('ir.module.category', string='Aplicaciones', compute="_compute_group_category_ids")
    ...
    def _compute_group_category_ids(self):
        for role in self:
            role.group_category_ids = role.trans_implied_ids.mapped('category_id')
